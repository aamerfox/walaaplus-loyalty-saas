...
| Events written **in the same transaction** as the action | **Supported now, and enforced by the database — EXACTLY, since migration `20260925130000`** | No event for an action that failed, and no action that survives a failed event — proved by making the event insert throw and asserting the redemption is not there. And no event for an action that finished earlier: the redemption stores the **transaction it was written in** (`pg_current_xact_id()`, server-assigned, not chooseable), and the trigger requires the current transaction to be that one. This replaces an earlier timestamp comparison that had a **one-millisecond window a retrying writer could walk through** — measured at 3 of 399 consecutive separate transactions. See §7. |
...
| **Backfilling historical events** | **Out of scope, and refused by the database** | No event exists for any redemption or void that happened before this migration, and none can be created afterwards. `occurredAt` is assigned by the trigger, so nobody can date one into the past — and the trigger requires the redemption's stored **transaction identity** to equal the current transaction's. A writer cannot imitate that: the identity is assigned by `pg_current_xact_id()` and whatever the caller supplies is discarded, and it is an exact 64-bit comparison rather than a timestamp with a resolution. Redemptions written before migration `20260925130000` carry no identity at all and are refused by name — they **fail closed**. |
