...
| **1a. Café pilot** | Stamp card only, one location, owner and cashier, direct QR enrollment, PWA card with service worker, scanner, phone lookup, award, redeem, reversal, customer and operation lists | Full café loop passes end-to-end; reconciliation clean; cashier restrictions enforced | One pilot café, real daily transactions, owner-defined minimum scan count, no unreconciled discrepancy |
| **1b. Merchant MVP** | Points cards and reward tiers, richer template UI, named UTM links, full staff and location management, real dashboard metrics | Points and stamp regression suite passes; dashboard derives from ledger | Two merchants using both card types for two continuous weeks |
| **1.5. Hardening** | Offline card cache, PWA telemetry, native VAPID push, staff-assisted restore, card and inactivity expiry, nightly reconciliation job, monitoring, backup drill | Security review, concurrency stress, backup restore evidence, staging deployment evidence, no critical or high open | 3–5 merchants, 30 consecutive days, minimum daily scan volume, zero unreconciled discrepancies, zero isolation failures, zero unresolved duplicate-award incidents |
