...
| **1b. Merchant MVP** | Points cards and reward tiers, richer template UI, named UTM links, full staff and location management, real dashboard metrics. **Prompt 1 ships the server and domain core, Prompt 2 the merchant UI and the Zademi brand system** ([PHASE-1B-IMPLEMENTATION.md](PHASE-1B-IMPLEMENTATION.md), [BRAND.md](BRAND.md)) | Points and stamp regression suite passes; dashboard derives from ledger | Two merchants using both card types for two continuous weeks |
...
| **3b. Developer platform** — **PARTIAL** | Public API, API keys, signed outbound webhooks, private GoHighLevel OAuth app, one validated POS connector. **Of these, only the internal event record and signed outbound webhooks are built** (Prompts 1-3, release-gated). The public API, API keys, the GHL app and a POS connector are **not started**; the last two are blocked on accounts nobody has created. Reconciled line by line in [PHASE-3B-SCOPE-RECONCILIATION.md](PHASE-3B-SCOPE-RECONCILIATION.md) | OAuth, token encryption, webhook signature and dedup, no duplicate loyalty writes from retries. **The webhook half passed** ([PHASE-3B-RELEASE-GATE.md](PHASE-3B-RELEASE-GATE.md)); the OAuth and duplicate-write criteria are untested because nothing tests them yet | Private GHL app tested in a real agency or location pilot — **not reachable**: no GHL app exists |
