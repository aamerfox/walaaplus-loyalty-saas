...
| An event cannot be backfilled, even by a direct writer | `walaaplus_validate_integration_event` requires the event's `occurredAt` to equal the named redemption's `recordedAt`. Both are assigned by their own triggers from `now()`, the transaction's start time, so equality means "same transaction" — and neither value can be chosen by the caller, because each is overwritten before it is read. Residual window: one millisecond, the resolution of `TIMESTAMP(3)` |
