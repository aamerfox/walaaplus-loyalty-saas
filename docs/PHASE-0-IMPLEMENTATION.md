...
| An integration event carries nothing about a person | `IntegrationEvent` has typed columns and **no JSON**: business, envelope version, event type, entity type, entity id, occurred-at, created-at, and nothing else. A column-name check in `tests/integration/integration-events-integrity.test.ts` fails on any column matching phone, email, name, code, digest, token, secret, wallet, amount, balance or url, and a service test serialises every stored event and searches it for the real phone, name, code, digest, salt, serial and both card tokens |
| An event describes something that actually happened, in the same tenant | `walaaplus_validate_integration_event` looks the entity up by `entityType`, and refuses a row whose entity does not exist, belongs to another business, or is the wrong kind for the event type — a foreign key would have accepted the cross-tenant row, which is why there is no foreign key on `entityId` |
| An event and the action it describes commit together | `emitIntegrationEvent` takes a transaction client and has no other overload, so a caller cannot emit outside the transaction that did the work; proved by making the insert throw and asserting neither the redemption nor the event exists |
| An event cannot be backfilled, even by a direct writer | `walaaplus_validate_integration_event` requires the named redemption's stored **transaction identity** (`writeXactId`, assigned from `pg_current_xact_id()`) to equal the current transaction's. Neither side can be chosen by the caller; both are overwritten before they are read. **No residual window** — it is an exact 64-bit comparison. A redemption predating migration `20260925130000` has no identity and is refused by name. *(Until that migration this was a `TIMESTAMP(3)` equality with a one-millisecond window that a retrying writer could walk through — see `docs/PHASE-3B-IMPLEMENTATION.md`.)* |
| An event's occurrence time is the server's | the trigger assigns `occurredAt` from `now() AT TIME ZONE 'UTC'` before anything reads it — ordering is the one thing a future consumer will trust, and a writer that chose it could place a withdrawal before the redemption it withdraws |
| Events are append-only and never delivered twice | `IntegrationEvent` is in `APPEND_ONLY_TABLES`; triggers refuse `UPDATE`, `DELETE` and `TRUNCATE` even for the owner; a unique index on `(eventType, entityId)` makes emission idempotent |
| A webhook URL and its signing secret are encrypted at rest | AES-256-GCM with a fresh 96-bit nonce per value, algorithm and key version in their own columns, and a CHECK constraint requiring the four-part envelope with an exact nonce and tag length — so a plaintext URL cannot be stored even by a direct writer. `INTEGRATION_ENCRYPTION_KEY`, per environment; **no plaintext fallback exists in any mode** |
| A missing encryption key stops webhooks and nothing else | `src/server/integrations/webhooks/crypto.ts` is imported only by the webhook code; enrolment, stamps, points, redemptions, referrals, consent, campaigns, the scanner, B7 and `/health` do not reach it. An integration test removes the key and redeems a coupon at the till successfully |
| A signing secret is shown once and never again | returned from `createDestination` and `rotateSecret` out of the value that generated it, never from a column; `DESTINATION_SELECT` omits both ciphertext columns, there is no reveal action, and the browser suite reloads the page and asserts the secret is gone |
| A merchant-supplied URL cannot reach this network | HTTPS only, no user-info, no IP literal, no loopback or private name; every resolved address validated, **and re-resolved immediately before each request** through the agent's own `lookup`, so the address checked is the address connected to. Redirects are refused rather than followed |
| An unsafe address is never retried | classified `PERMANENT`; the delivery trigger refuses a `FAILED` row carrying `UNSAFE_ADDRESS`, so it can never be reached by exhausting retries |
| A timeout or network error is never recorded as delivered | the trigger requires a `DELIVERED` delivery to carry a 2xx status and no error class |
| No response body, header, URL or raw error is ever stored | the three webhook tables have typed columns and no JSON; the transport keeps only `res.statusCode` and the error CLASS, chosen from an error code rather than from message text |
| Outbound HTTP exists in exactly one module, called only by the worker | `webhooks/transport.ts`; `tests/unit/webhook-boundary.test.ts` asserts it is the only file under `src/server` holding an HTTP client and that nothing under `src/app/` imports it or the delivery runner |
| Nothing in the product can reach a provider | no `fetch`, HTTP client, queue, worker, timer, provider SDK, URL, API key, token, SMTP setting, encryption key, HMAC or environment variable exists under `src/server/integrations/`, asserted by reading the source in `tests/unit/integration-boundary.test.ts`; there is no `/api/integrations`, `/api/webhooks` or `/api/events` route |
