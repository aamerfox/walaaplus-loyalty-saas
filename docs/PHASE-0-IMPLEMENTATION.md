...
| Six tables are append-only under the same two layers: the ledger, consent history, campaign revisions, campaign approvals, audience snapshots and their members | `APPEND_ONLY_TABLES` in `scripts/db-roles.mjs` grants only `SELECT`/`INSERT`; a PL/pgSQL trigger per table refuses even the owner. Both refusals are asserted in `tests/integration/runtime-role.test.ts` and in the feature suites |
