...
Updated at Phase 1b Prompt 2. One row per capability, and the "Where" column points at the code so a
...
| Named UTM source attribution | **Built, internal only** — created, listed, activated server-side; every card carries its source | `src/server/program/source-links.ts` |
...
| Editing a live card template | **A live program's rules are frozen** | A card keeps the rules it was sold under. Changing them means publishing a new version — the contract for that is not built yet (see below) |

## Not built, and which phase owns it

| Capability | Phase |
|---|---|
| Publishing a new program version (the contract that makes an edit possible) | 1b, a later prompt |
| Creating and renaming locations | 1b, a later prompt |
| Named-source creation and deactivation from the UI | 1b, a later prompt |
