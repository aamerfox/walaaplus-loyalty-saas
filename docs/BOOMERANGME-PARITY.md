...
| Outbound webhooks with signing | **Built (Phase 3B Prompt 2)** — owner-only HTTPS destinations, AES-256-GCM at rest, HMAC-SHA-256 over `timestamp.body`, SSRF and DNS-rebinding defence with the address re-resolved before every request, bounded retries, an append-only attempt history, and delivery from the worker only. At-least-once, stated in the UI. **Two event types, no named provider** | `src/server/integrations/webhooks/`, `/business/integrations` |
...
| Integrations | An append-only internal event record for two finished workflows, a versioned envelope, database-enforced tenant and semantic integrity, an owner/manager read view, **and (Prompt 2) a signed outbound webhook to an owner-supplied HTTPS address** | **Everything that connects to a named provider.** No email, SMS, WhatsApp, Meta, Google, Stripe, PayPal or POS integration, no OAuth, no SDK, no inbound webhook and no public API — and no place to put a provider credential, which is **D27**. Two event types out of the reference product's ~40, and each addition is **D28**. No backfill, and no key-rotation tool (**D29**) or retention rule (**D30**) |
