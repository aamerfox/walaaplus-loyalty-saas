...
| Integrations | An append-only internal event record for two finished workflows, a versioned envelope, database-enforced tenant and semantic integrity, an owner/manager read view, **a signed outbound webhook to an owner-supplied HTTPS address (Prompt 2), delivered by an isolated egress service (Prompt 3), and a release-gate audit of the whole feature** | **Everything that connects to a named provider.** No email, SMS, WhatsApp, Meta, Google, Stripe, PayPal or POS integration, no OAuth, no SDK, no inbound webhook and no public API — and no place to put a provider credential, which is **D27**. Two event types out of the reference product's ~40, and each addition is **D28**. No backfill, and no key-rotation tool (**D29**) or retention rule (**D30**) |
