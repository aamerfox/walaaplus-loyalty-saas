...
| D27 | **Where a provider credential would live, and who may read it.** Phase 3B Prompt 1 built the internal event record and deliberately added **no** secrets table, **no** generic JSON configuration column, **no** master encryption key, **no** environment variable and **no** deployment change — because every integration family in `docs/INTEGRATIONS-CAPABILITY-MATRIX.md` needs at least one secret, and where it lives is a deployment decision with a backup-and-restore story attached. Five sub-questions, none guessed: where the ciphertext lives; where the key lives; who may read a credential back (the honest answer is nobody, as with a coupon code); what happens on rotation; and whether a credential is per business or per platform — which decides whether a merchant brings their own account or rides on ours, and is commercial rather than technical. **Every outbound integration is blocked on this.** | ⬜ |
| D28 | **Which workflows may ever emit an event.** Two exist: a promotion redemption and its withdrawal, both already-finished workflows that grant nothing. The reference product has roughly forty. Each addition is a decision about what a workflow may tell the outside world — an enrolment event, for instance, would tell a future consumer that a named card came into existence at a moment, which is a different disclosure from "an offer was recorded". Nothing is blocked; the list simply does not grow by convenience. | ⬜ |
| D29 | **Re-encrypting webhook secrets when `INTEGRATION_ENCRYPTION_KEY` rotates.** Every ciphertext carries its key version, so a rotation can write version 2 beside version 1 rows and read both — the schema is ready for it. **The re-encryption tool is not built.** Until it is, rotating the key makes every existing destination undecryptable, which fails closed (deliveries stop, nothing leaks) but is not a rotation anybody would want to perform. The owner decides when a rotation is a real operational need; building it is a prompt, not an afternoon. | ⬜ |
| D30 | **Webhook delivery retention.** Nothing deletes a delivery or an attempt, and a revoked destination keeps its history — what this business sent, and to whom, is a record. That leaves a retention period open, and what happens to the history when a customer asks to be erased: a delivery names an event, which names a redemption, which names a person. Related to D9, D13, D20 and D23. | ⬜ |
| D31 | **API-key retention after expiry or revocation.** Phase 3B.1 Prompt 1 issues keys that expire after ninety days and can be revoked at any time, and **nothing deletes the row** — the record that a business held a credential between two dates is exactly what an audit trail is for, and the database refuses a DELETE for the runtime role and the table owner alike. What that leaves open is a retention period, and what happens to the record when a business closes. Nothing is blocked by it: the rows hold no customer data and no recoverable secret, only a digest that cannot be reversed. Related to D9, D13, D20, D23 and D30, which are the same question about other append-only records. | ⬜ |
| D32 | **Whether an owner is warned before an API key expires, and how.** Phase 3B.1 Prompt 2 ships a key that stops working ninety days after it is created. The expiry date is on the owner's screen and **nothing else happens**: no email, no SMS, no banner, no reminder. So the realistic failure is silent — a merchant's reporting job stops one morning, the key list says why, and nobody was looking at the key list. The decision is not whether ninety days is right (the owner set it) but whether a warning is owed, and through what: a dashboard banner needs no new capability and could ship next; an email or SMS reminder would need a provider, which is **D27**, and would be the first outbound message this product has ever sent. Doing nothing is a defensible answer for a pilot with a handful of merchants, and it is the current answer — it is recorded here so it is a choice rather than an oversight. | ⬜ |
| D33 | **Widening the pre-existing `int4` money columns.** Every money column written before Phase 4 is a PostgreSQL `integer`: `LoyaltyOperation.purchaseAmountMinor`, `monetaryDeltaMinor`, `redemptionValueMinor`, and `CustomerCard.cashBalanceMinor`. That caps them at 2,147,483,647 minor units — about 21 million SYP — which a real customer's purchase already exceeds, and `purchaseAmountMinor` is **live**: the scanner writes it on every points award. Phase 4's own columns are all `BIGINT` and the cashback balance is derived from the chain rather than from `cashBalanceMinor`, so nothing new is at risk. What is left open is the existing data: widening a live column is its own migration with its own lock behaviour and its own deployment window, and doing it quietly inside a feature prompt would be the wrong way to find out. Until it is done, a points award above the cap fails at the database rather than silently wrapping. | ⬜ |
| D34 | **Fixed-amount discounts (“200 off”, not “5% off”).** Phase 4 ships percentage rates only. A fixed amount is not a smaller version of the same feature: it needs its own cap rule (a 200 discount on a 150 bill), its own tier semantics (thresholds in the same unit as the discount, which invites the two being confused), and a different answer to what happens when a merchant changes currency. Every one of those is a merchant-facing rule rather than an implementation detail. Deferring it is recorded here rather than left as an apparent oversight; `docs/PHASE-4-MONEY-MATRIX.md` §6.3 carries the reasoning. | ⬜ |
| D35 | **Whether today's invoice counts towards the tier that prices it.** The tier is chosen from the card's cumulative qualified spend **before** the current invoice, so a customer who has spent 900,000 and presents a 200,000 bill earns at the 900,000 rate, and the 200,000 counts next time. The alternative — include the current invoice, so a single large purchase can reach a higher band immediately — is equally arguable and pays a different amount of money. This reading was chosen because a cashier can explain it and because it is monotonic: a customer's rate never depends on how a purchase happened to be split across receipts. It is **implemented, documented and asserted by a test**, not assumed silently; overturning it changes one line and nothing else in the design. `docs/PHASE-4-MONEY-CONTRACT.md` §6. | ⬜ |
| D36 | **Whether a role should exist that can earn but not redeem.** Phase 4 maps each money action to the permission its non-money equivalent already needs: earning to `MAKE_ACCRUALS`, redemption and discount to `MAKE_REDEMPTIONS`, reversal to both. The database and the services enforce that split. But `effectivePermissions` is *role defaults ∪ explicit grants* — the membership column **adds**, it never restricts — and every `CASHIER` holds both permissions by default. So **no role this product currently defines can apply a discount without also being able to award, or vice versa**, and the separation is real in the code and absent in practice. Whether a merchant actually wants a till that can give cashback but not spend it is a staffing question, not an engineering one. | ⬜ |
| D37 | **Owner money-input convention.** Thresholds and rates in owner-facing money forms are entered as exponent-aware decimals in the business's own currency, consistent with `RateTableEditor`; shared string parsers convert thresholds to minor units and rates to basis points, refusing excess precision rather than rounding. The business currency and exponent are display facts, never form inputs. | ✅ decided — approved for Prompt 3 |
