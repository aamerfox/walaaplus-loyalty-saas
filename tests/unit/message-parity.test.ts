...
describe("message files", () => {
  ...
  it("translates the Arabic, apart from the few strings that are identical on purpose", () => {
    /*
     * A string that is byte-identical in both files is usually an untranslated copy. Six here are
     * legitimately identical and are listed so a seventh has to be argued for: a placeholder-only
     * subtitle, two phone placeholders that are digits, the product name — which is the same word
     * in every locale, because a translated product name is a different product — and two wallet
     * headings that name a file format and an API object rather than saying anything.
     */
