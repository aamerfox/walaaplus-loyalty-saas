...
describe("the database holds the partial form, not the unconditional one", () => {
  it("is a partial unique index predicated on ACTIVE", async () => {
    /*
     ...
     * express a partial index so it deliberately disagrees.
     */
    const [index] = await migratorPrisma().$queryRawUnsafe<{ indexdef: string }[]>(
      `SELECT indexdef FROM pg_indexes WHERE schemaname = 'public' AND indexname = 'ApiKey_businessId_name_key'`,
    );
    expect(index, "the index must still exist").toBeTruthy();
